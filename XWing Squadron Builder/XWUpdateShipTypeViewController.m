//
//  XWUpdateShipTypeViewController.m
//  XWing builder
//
...
@interface XWUpdateShipTypeViewController ()

@end

@implementation XWUpdateShipTypeViewController
...
- (void)viewDidLoad
{
    ...
    if (self.shiptype) { // Update Text Field
        [self.nameTextField setText:self.shiptype.name];
    }
}
...
- (IBAction)save:(id)sender {
    ...
    if (aname && aname.length) { // if a name has been entered
        // Populate Record
        self.shiptype.name=aname; // save will be done when quitting application (see AppDelegate)
        // Pop View Controller
        [self.navigationController popViewControllerAnimated:YES];
    } else {
        // Show Alert View
